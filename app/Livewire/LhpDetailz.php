...
class LhpDetail extends Component
{
    ...
    public function deleteFile($fieldName)
    {
        try {
            ...
            if ($this->lhp->$fieldName && Storage::disk($disk)->exists($this->lhp->$fieldName)) {
                Storage::disk($disk)->delete($this->lhp->$fieldName);
            }
        ...
        try {
            ...
            if ($this->tindakLanjutFile) {
                ...
                if ($this->tindakLanjutId) {
                    ...
                    if ($existing && Storage::disk($disk)->exists($existing->file_path)) {
                        Storage::disk($disk)->delete($existing->file_path);
                    }
                }

                // Store new file
                $filePath = $this->tindakLanjutFile->store('tindak-lanjut', $disk);
    ...
    public function deleteTindakLanjut($id)
    {
        try {
            ...
            // Delete file from storage
            if (Storage::disk($disk)->exists($tindakLanjut->file_path)) {
    ...
    public function downloadTindakLanjut($id)
    {
        ...
        return Storage::disk($disk)->download($tindakLanjut->file_path, $tindakLanjut->file_name);
    }
