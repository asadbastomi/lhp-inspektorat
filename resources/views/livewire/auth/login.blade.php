            ...
            <form wire:submit="login" class="flex flex-col gap-6 mt-4">
                    ...
                    <div class="flex justify-between items-center mb-1">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" wire:navigate
                                class="text-sm text-[#0277BD] hover:underline">Lupa kata sandi?</a>
                        @endif
                    </div>
                    <input wire:model="password" id="password" type="password" required autocomplete="current-password"
                        placeholder="Kata Sandi" class="form-input">
                    @error('password')
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex items-center fade-in-up" style="animation-delay: 400ms;">
                    <input wire:model="remember" id="remember" type="checkbox"
                        class="form-checkbox h-4 w-4 text-[#1B5E20] focus:ring-[#388E3C]">
                    <label for="remember" class="ml-2 block text-sm text-gray-900">Ingat saya</label>
                </div>

                <div class="fade-in-up" style="animation-delay: 500ms;">
                    <button type="submit" class="btn btn-primary w-full flex justify-center">
            ...
            @if (Route::has('register'))
                <div class="text-center text-sm text-gray-600 mt-auto fade-in-up" style="animation-delay: 600ms;">
                    ...
                    <a href="{{ route('register') }}" wire:navigate
                        class="font-medium text-[#0277BD] hover:underline">Daftar</a>
                </div>
            @endif
        </div>
    </div>
        ...
        <div class="absolute inset-0 flex items-center justify-center p-12">
            <div class="fade-in" style="animation-delay: 200ms;">
                <svg class="w-full h-auto" viewBox="0 0 512 384" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M72 128L168 200L264 128L360 184L440 104" stroke="#10B981" stroke-width="8"
