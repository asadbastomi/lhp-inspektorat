<div class="container mx-auto px-4 py-8">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr wire:key="{{ $jabatan->id }}" class="hover:bg-gray-50/70 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-[--color-dark]">{{ $jabatan->jabatan }}</div></td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end gap-4">
                            <button wire:click="edit('{{ $jabatan->id }}')" class="text-gray-500 hover:text-[--color-secondary] transition" title="Edit"><i class="fas fa-edit"></i></button>
                            <button wire:click="delete('{{ $jabatan->id }}')" wire:confirm="Anda yakin ingin menghapus data Jabatan ini?" class="text-gray-500 hover:text-[--color-accent] transition" title="Hapus"><i class="fas fa-trash"></i></button>
                        </div>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center">
                        <div class="text-center">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-[--color-dark]">Tidak ada data Jabatan ditemukan</h3>
    ...
    <div x-show="$wire.isModalOpen" x-on:keydown.escape.window="$wire.closeModal()" x-trap.inert.noscroll="$wire.isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="card relative w-full max-w-lg bg-white p-8" x-show="$wire.isModalOpen"
            ...
            <h3 class="text-2xl font-bold text-[--color-dark] mb-6">{{ $jabatan_tim_id ? 'Edit' : 'Tambah' }} Data Jabatan Tim</h3>
            <form wire:submit.prevent="store" class="space-y-6">
                <div>
                    <label for="jabatan_tim" class="block text-sm font-medium text-[--color-dark] mb-1">Nama Jabatan Tim</label>
                    <input type="text" id="jabatan_tim" wire:model="jabatan_tim" x-ref="focusInput" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-[--color-accent] focus:border-[--color-accent]">
                    @error('jabatan_tim') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>
